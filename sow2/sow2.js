...
`;
    ...
    function showRemoveAccountModal(email) {
      ...
      document.getElementById("confirmRemoveAccountBtn").onclick = async () => {
        rmOverlay.remove();
        try {
          const resp = await fetch(`${window.API_BASE_URL}/gdrive/sow2/auth/remove`, { method: "POST", headers: { Authorization: `Bearer ${localStorage.getItem("access_token")}`, "X-User-Email": email } });
          if (resp.ok) {
            const localKeys = [
              "gdrive_user_email",
...
function pickerCallback(data) {
  if (data.action === google.picker.Action.PICKED) {
    ...
    localStorage.setItem("sow2_folder_name", folder.name);
...
async function openUploadModal() {
  ...
  const _lastFolderId = getSow2FolderId();
  const _lastFolderName = localStorage.getItem("sow2_folder_name");
...
async function loadSow2Files() {
  try {
    ...
    const cached = localStorage.getItem('cache_sow2_files_scoped');
    if (cached) {
      try {
        allSow2Files = JSON.parse(cached);
        populateTopicFilter();
        applyFiltersAndDisplay();
    ...
    try { localStorage.setItem('cache_sow2_files_scoped', JSON.stringify(allSow2Files)); } catch(e) {}
    populateTopicFilter();
    applyFiltersAndDisplay();
  } catch (error) {
    ...
    if (error.name === 'AbortError') {
      contentList.innerHTML = `<div class="empty-state"><i class="fas fa-satellite-dish empty-icon"></i><p>Server is starting up &mdash; this can take up to 30 seconds on first load.</p><button onclick="loadSow2Files()" style="margin-top:12px;padding:8px 20px;background:#3a4d2c;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:0.9rem;">&#8635; Retry</button></div>`;
    } else {
